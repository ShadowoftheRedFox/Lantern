...
/**@type {{ [id: number]: { x: number, y: number, date: number, id: number } }} */
MouseTrackerManager.lastMove = {};
/**@type {{ [id: number]: { x: number, y: number, date: number, id: number } }} */
MouseTrackerManager.trueMove = {};
/**@type {{ [id: number]: { x: number, y: number, date: number, id: number } }} */
MouseTrackerManager.spawnCoos = {};
...
MouseTrackerManager.OnTouchStart = function (event) {
    ...
    for (let i = 0; i < event.changedTouches.length; i++) {
        ...
        MouseTrackerManager.spawnCoos[event.changedTouches.item(i).identifier.toString()] = {
            x: event.changedTouches.item(i).clientX,
            y: event.changedTouches.item(i).clientY,
            date: Date.now(),
            id: event.changedTouches.item(i).identifier
...
MouseTrackerManager.OnTouchEnd = function (event) {
    for (let i = 0; i < event.changedTouches.length; i++) {
        const id = event.changedTouches.item(i).identifier.toString();
        delete MouseTrackerManager.lastMove[id];
        delete MouseTrackerManager.trueMove[id];
        delete MouseTrackerManager.spawnCoos[id];
    }
...
MouseTrackerManager.OnTouchMove = function (event) {
    ...
    for (let i = 0; i < event.changedTouches.length; i++) {
        const id = event.changedTouches.item(i).identifier.toString();
        MouseTrackerManager.lastMove[id].x = event.changedTouches.item(i).clientX;
        MouseTrackerManager.lastMove[id].y = event.changedTouches.item(i).clientY;
        MouseTrackerManager.lastMove[id].date = Date.now();
        MouseTrackerManager.trueMove[id] = MouseTrackerManager.lastMove[id];
        // there is no hover, so kinda useless, just stays here just in case
        // MouseTrackerManager.stopedMoved(MouseTrackerManager.lastMove[id]);
    }
};
...
MouseTrackerManager.OnMouseMove = function (event) {
    ...
    MouseTrackerManager.trueMove["0"] = MouseTrackerManager.lastMove["0"];
...
MouseTrackerManager.stopedMoved = function (old) {
    ...
    if (MouseTrackerManager.moving === true) {
        setTimeout(() => {
            // if the cursor did not move in the frame interval
...
MouseTrackerManager.OnMouseClick = function (event) {
    MouseTrackerManager.holding = true;
    MouseTrackerManager.spawnCoos["0"] = MouseTrackerManager.trueMove["0"];
};
...
MouseTrackerManager.checkOver = function (x, y, w, h, old = false) {
    if (Utils.isMobileDevice() === false) {
        const l = MouseTrackerManager.lastMove["0"] || { x: -100, y: -100 },
            o = MouseTrackerManager.trueMove["0"] || { x: -100, y: -100 };
